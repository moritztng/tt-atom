"""``Calculator`` — the single public entry point across the model families.

One call, model picked by name, no need to know whether it is UMA, Orb, or PET-MAD (mirrors
fairchem's ``FAIRChemCalculator`` and Hugging Face's ``AutoModel.from_pretrained``)::
    ...
    atoms.calc = Calculator(atoms, "pet-mad-s-v1.5.0")          # a PET-MAD checkpoint
...
``pet-*`` to the PET-MAD (UPET) backbone
(:meth:`tt_atom.pet_calculator.PETCalculator.from_checkpoint`, likewise name-is-checkpoint).
Everything downstream is plain ASE either way.

The families genuinely differ, so a family-inapplicable argument raises rather than being
...
conditioning and ignore them, as documented; PET-MAD has no conditioning at all). Reach for the
classes / classmethods directly (``TTAtomCalculator``, ``OrbCalculator``, ``PETCalculator``,
``.from_uma``, ``.from_checkpoint``) only when you want to pin a UMA task, hand in a prebuilt
bundle, or manage the weight file yourself.
"""
from __future__ import annotations


def _family(model):
    """Map a model name to its family (``"uma"`` / ``"orb"`` / ``"pet"``); raise on anything else."""
    m = str(model).lower()
    if m.startswith("uma"):
        return "uma"
    if m.startswith("orb"):
    ...
    raise ValueError(
        f"unknown model {model!r}: expected a UMA model (e.g. 'uma-s-1'), an Orb checkpoint "
        f"(one of {', '.join(OWC.CHECKPOINTS)}), or a PET-MAD checkpoint "
        f"(one of {', '.join(__import__('tt_atom.pet_weight_cache', fromlist=['CHECKPOINTS']).CHECKPOINTS)}).")
...
def Calculator(atoms=None, model="uma-s-1", *, task=None, charge=0, spin=1, refenv=None,
    ...
    if _family(model) == "pet":
        from .pet_calculator import PETCalculator

        if task is not None:
            raise ValueError(
                f"`task` applies only to UMA models; the PET-MAD checkpoint {model!r} has no task "
                "selector. Drop task=.")
        if checkpoint is not None:
            raise ValueError(
                f"for PET-MAD the model name is the checkpoint (model={model!r}); there is no "
                "separate raw-checkpoint override to pass. Drop checkpoint=.")
        if trace:
            raise ValueError(
                "trace= is not wired into the PET-MAD calculator; the per-structure forward is "
                "not yet trace-captured (a later pass, see skill ttnn-trace-capture).")
        if atoms is not None:
            atoms.info.setdefault("charge", charge)
            atoms.info.setdefault("spin", spin)
        return PETCalculator.from_checkpoint(checkpoint=model, refenv=refenv, cache_dir=cache_dir,
