...
class OrbCalculator(DeviceCalculator):
    def __init__(self, weights, device=None, device_id=0, fast=False, bucketing=False, **kwargs):
        ...
        from .orb_model import (AttentionInteractionLayer, Encoder, EnergyHead, ForceHead,
                                StressHead)
        ...
        self.layers = [AttentionInteractionLayer(w, f"gnn_stacks.{i}", self.device,
                       ...
                       for i in range(L)]
        self.ehead = EnergyHead(w, self.device, latent_dim=latent_dim, fast=fast)
        ...
        self.fhead = (ForceHead(w, self.device, latent_dim=latent_dim, fast=fast)
                      if self.is_direct else None)
        self.shead = (StressHead(w, self.device, latent_dim=latent_dim, fast=fast)
                      if "stress_head.mlp.NN-0.weight" in w else None)
        ...
        if self.is_direct:
            ...
            if self.bucketing:
                dev_senders, dev_receivers, cutoff, gkw, e_bucket = pad_graph(
                    senders, receivers, cutoff, max_num_neighbors=self.max_num_neighbors)
            ...
            raw_e, F_raw, *rest = energy_and_forces(
                ...
                bucketing=self.bucketing, max_num_neighbors=self.max_num_neighbors)
            E = host_energy_denormalize(
