"""Multi-card fan-out parity for the Orb family — the counterpart to ``tests/test_multicard.py``'s
UMA test. ``MultiCard`` sharding independent systems across N cards must give the SAME per-system
energies, in the SAME order, as running every system sequentially on one card.

Each system is evaluated by exactly one worker in exactly one code path (the ``_run_orb`` loop in
...
disjoint-union batching's bf16 accumulation-order sensitivity, see ``test_orb_disjoint_batch.py``)
that could make the result depend on the shard layout. This test exercises the real
queue/dispatch/gather path (``MultiCard.energies``), not just the per-system compute.

Requires the real ``orb-v3-conservative-inf-omat`` weight cache (built once via the reference env,
...
pytestmark = [
    pytest.mark.skipif(not WEIGHTS.exists(), reason="orb weight cache (conservative-inf-omat.npz) not found"),
